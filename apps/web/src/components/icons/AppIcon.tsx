...
export function AppIcon({ appId, size = 48, className = "" }: AppIconProps) {
  ...
  // ==========================================
  // 1. macOS BIG SUR / SONOMA (Sleek Squircles)
  // ==========================================
  if (osStyle === "macos") {
    return (
      <div
      ...
      >
        ...
        {appId === "terminal" && (
          <div className="w-full h-full bg-gradient-to-b from-zinc-700 to-zinc-950 flex items-center justify-center border border-white/10">
            <span className="font-mono font-bold text-emerald-400 text-xs tracking-tighter">&gt;_</span>
          </div>
        )}
        {appId === "notes" && (
        ...
        {appId === "settings" && (
          <div className="w-full h-full bg-gradient-to-b from-slate-500 to-slate-700 flex items-center justify-center border border-white/20">
            <Settings className="w-1/2 h-1/2 text-white animate-spin-slow" />
          </div>
        )}
        {appId === "trash" && (
          <div className="w-full h-full bg-gradient-to-b from-rose-500 to-rose-700 flex items-center justify-center border border-white/20">
            <Trash2 className="w-1/2 h-1/2 text-white drop-shadow-sm" />
          </div>
        )}
        {appId === "gmail" && (
          <div className="w-full h-full bg-gradient-to-b from-red-500 to-red-600 flex items-center justify-center border border-white/20">
            <Mail className="w-1/2 h-1/2 text-white drop-shadow-sm" />
          </div>
        )}
        {appId === "telegram" && (
          <div className="w-full h-full bg-gradient-to-b from-sky-400 to-blue-500 flex items-center justify-center border border-white/20">
  ...
  // ==========================================
  // 2. WINDOWS 7 AERO GLASS (Skeuomorphic Shine)
  // ==========================================
      ...
      >
        ...
        {appId === "explorer" && (
          <div className="w-full h-full bg-gradient-to-b from-amber-300 via-amber-400 to-amber-600 flex items-center justify-center border border-white/60 shadow-inner">
            <Folder className="w-3/5 h-3/5 text-amber-950 fill-amber-100/60 drop-shadow" />
          </div>
        )}
        {appId === "terminal" && (
        ...
        {appId === "gmail" && (
          <div className="w-full h-full bg-gradient-to-b from-rose-400 via-red-500 to-red-700 flex items-center justify-center border border-white/60 shadow-inner">
            <Mail className="w-3/5 h-3/5 text-white drop-shadow" />
          </div>
        )}
        {appId === "telegram" && (
          <div className="w-full h-full bg-gradient-to-b from-sky-300 via-blue-400 to-blue-600 flex items-center justify-center border border-white/60 shadow-inner">
            <MessageSquare className="w-3/5 h-3/5 text-white fill-white/20 drop-shadow" />
          </div>
        )}
      </div>
    );
  ...
  // ==========================================
  // 3. WINDOWS 95 RETRO (3D Beveled Pixel Art)
  // ==========================================
  if (osStyle === "win95-retro") {
    return (
      ...
      >
        ...
        {appId === "terminal" && (
          <div className="w-4/5 h-4/5 bg-black border border-zinc-600 flex items-center justify-center">
            <span className="font-mono font-bold text-white text-xs">&gt;_</span>
          </div>
        )}
  ...
  // ==========================================
  // 4. WINDOWS 11 MODERN (Default Fluent Acrylic)
  // ==========================================
  return (
    <div
      style={{ width: size, height: size }}
